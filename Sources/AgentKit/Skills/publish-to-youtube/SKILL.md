...
3. Pick the preset from the sequence's own shape, not from habit. Read the active sequence's `orientation` and the clips' `displaySize` (rotation applied; the encoded `size` answers orientation backwards for phone footage). Use `h264_1080p` for a landscape sequence and `reel9x16` for a portrait one, and note that a preset whose shape differs from the sequence's is letterboxed on black — the compositor aspect-fits, it never crops. If `project_describe` returned a `formatMismatch`, the sequence itself is the wrong shape for its footage and exporting now bakes the bars into the upload: tell the user and offer to resize the sequence with `setSequenceSettings` first. Then `render_export` with the chosen preset. It answers `status: "approval_required"`; tell the user you are waiting for their approval and retry the identical call with the returned `approvalToken` once granted. Keep the `renderId` from the done answer.
