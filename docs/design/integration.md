...
| Publishing: `accounts[.google]` | `PublishKit.GoogleAccountProvider` | `PublishingServices` (`Sources/TimelineApp/Services/`): the OAuth client from `GoogleClientConfiguration.resolve` (D3 lookup order), set in the Publishes panel through `GoogleClientFile` and installed live by `PublishingServices.apply` (docs/plans/publish-client-setup.md), the token store from `TokenStoreSelection.resolve` (the 0600 file `google-tokens.json` for the unsigned binary, the Keychain from an `.app`), `accounts.json` next to it, the browser opened by `WorkspaceAuthorizationPresenter` (`NSWorkspace.shared.open`; AppKit stays in the app), PublishKit's loopback listener taking the redirect. Without a client the provider is registered unconfigured, so the panel shows the empty client card, `publish_youtube` and `publish_status` are unregistered until one is saved, and `account_status` answers `configured: false` |
...
A drag out of the library panel hands its rows across in-process (`LibraryDragPayload.inFlight`, set by
`MediaLibraryModel.dragProvider` and read by `LibraryDragPayload.items(on:)`), because the pasteboard
cannot be relied on: everything registered on an `NSItemProvider` — the payload's bytes and the file URL
alike — is promised through SwiftUI's drag bridge and resolved asynchronously, so a drop reading
`pasteboard.data(forType:)` gets whatever has arrived by then. Often that is **zero bytes** (the type is
