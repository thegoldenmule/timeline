...
| `TimelineUI` | Library of views: timeline `NSViewRepresentable` Metal view (ruler, tracks, clips with link-group and transition rendering, filmstrips via `ThumbnailProvider`, waveforms via `WaveformProvider`, playhead, trim/move/split gestures with local preview and one command on release, snapping, ripple/overwrite modifier); the shared panel chrome and tokens (`PanelTheme`, `PanelChrome`, `PanelLayoutModel` — `docs/design/ui-style.md`); inspector; approval cards; job progress | Core, Contracts (fakes) | fakes + snapshot tests | Renders fixture projects including linked clips and a transition; each gesture emits exactly one `Command` to the fake store on release; undo/redo drives history from the fake; 60 fps at 1,000 clips across 5 zoom levels; the approval card round-trips an `approval_required` result; no literal padding, radius, or font in a panel |
