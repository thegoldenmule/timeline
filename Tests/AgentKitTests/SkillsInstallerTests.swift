...
@Suite struct SkillsInstallerTests {
    @Test func bundledSkillsSequenceTheTools() throws {
        ...
        #expect(skills.map(\.name) == ["jump-cut-talking-head", "sync-daw-mix", "tiktok-captions"])
        for skill in skills {
            ...
            #expect(body.contains("project_describe"), "\(skill.name) starts by reading the project")
    ...
    @Test func installsIntoTheWorkingDirectory() throws {
        ...
        #expect(reread.map(\.name) == ["jump-cut-talking-head", "sync-daw-mix", "tiktok-captions"])
    }
